import os
import glob
import platform  # Add OS detection logic
...
def get_ttp_list(filter_by_os=True):
    ...
    """
    scripts = []
    current_os = platform.system() if filter_by_os else None  # Detect the OS if filtering is enabled

    for script in sorted(glob.glob(os.path.join(RED_TTP_PATH, "*.py"))):
        # Exclude __init__.py and common.py
        if os.path.basename(script) not in ("__init__.py", "common.py"):
            # Filter by OS if enabled
            if filter_by_os:
